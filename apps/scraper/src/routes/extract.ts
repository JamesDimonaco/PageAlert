...
extractRoutes.post("/", zValidator("json", extractSchema), async (c) => {
  ...
  try {
    ...
    return c.json({
      ...
      // hashContent takes the local rather than scraped.text: the pre-scraped
      // path never calls scrapeUrl, and the fingerprint has to cover both or
      // the scheduler's unchanged short-circuit silently stops working.
